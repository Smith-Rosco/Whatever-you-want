...
(function runMergedCustomUIWithDOMWatcher() {
    ...
    if (!window.DOMWatcherService) {
        console.error("【自定义脚本】错误: DOMWatcherService 未加载！脚本无法运行。请确保 DOMWatcherService.js 在此脚本之前加载。");
        // 如果 DOMWatcherService 不存在，可以考虑回退到旧的 MutationObserver 逻辑或直接停止
        // 为了演示，这里我们直接返回
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', () => {
                console.error("【自定义脚本】DOMContentLoaded 后检测：DOMWatcherService 仍然未加载！");
            });
        }
        return;
    }
    ...
    function setupCustomControls(controlsAreaElement) {
        if (!controlsAreaElement) {
            console.warn('【自定义脚本】setupCustomControls: 控制区域元素无效。');
            return;
        }
        ...
        if (!modeToggleButton) {
            ...
            console.log('【自定义脚本】模式切换按钮已创建并绑定事件。');
        }
        ...
        if (!enhancementToggleButton) {
            ...
            console.log('【自定义脚本】增强切换按钮已创建并绑定事件。');
        }
        ...
        if (sendButtonElement) {
            ...
            if (modeToggleButton.parentElement !== controlsAreaElement) {
                 controlsAreaElement.insertBefore(modeToggleButton, sendButtonElement.nextSibling);
            }
            if (enhancementToggleButton.parentElement !== controlsAreaElement) {
                 controlsAreaElement.insertBefore(enhancementToggleButton, modeToggleButton.nextSibling);
            }
            console.log('【自定义脚本】自定义按钮已尝试添加到原生发送按钮之后。');
        } else {
            // 如果找不到发送按钮，追加到控制区域末尾
            if (modeToggleButton.parentElement !== controlsAreaElement) {
            ...
            console.warn('【自定义脚本】未在控制区域找到原生发送按钮 (' + NATIVE_SEND_BUTTON_SELECTOR + ')，自定义按钮已尝试追加到控制区域 (' + NATIVE_CONTROLS_AREA_SELECTOR + ') 的末尾。');
        }
    }
    ...
    activeOptionClickListener = function (event) {
        ...
        if (!currentOptParent) {
            console.warn('【自定义脚本】[选项点击] 警告: 找到了 li，但未能找到其所属的父级 <opt> 元素 (' + OPTIONS_PARENT_SELECTOR + ')。');
            return;
        }
        ...
        console.log(`【自定义脚本】[选项点击] 事件触发于 <opt> (是否为最后一个: ${isLastOptParent}). 目标 li:`, clickedLiElement.textContent.trim());
        ...
        if (optionsInThisArea.length === 0) {
            console.warn('【自定义脚本】[选项点击] 警告: 在当前选项区 ('+ OPTIONS_AREA_SELECTOR +') 内未找到任何选项元素 (使用选择器 ' + OPTION_ITEM_SELECTOR + ')。');
            return;
        }
        ...
        if (isLastOptParent && optionsInThisArea.length === 4) {
            if (clickedIndexInThisArea === 0 || clickedIndexInThisArea === 1) {
                currentMode = '角色扮演模式';
                console.log('【自定义脚本】[选项点击][最后一个选项区] 模式因选项点击(当前区索引0或1)切换为: 角色扮演模式');
            } else if (clickedIndexInThisArea === 2 || clickedIndexInThisArea === 3) {
                currentMode = '小说模式';
                console.log('【自定义脚本】[选项点击][最后一个选项区] 模式因选项点击(当前区索引2或3)切换为: 小说模式');
            }
            updateModeButtonDisplay(); // 更新按钮显示
        } else if (optionsInThisArea.length === 4) {
            console.log(`【自定义脚本】[选项点击] 当前选项区 (非最后一个，但有4个选项) 被点击。点击索引: ${clickedIndexInThisArea}。未执行与 "最后一个选项区特定" 的模式自动切换逻辑。`);
            // 如果希望所有4选项的区域都切换模式，可以取消 isLastOptParent 的限制，或者添加额外逻辑
        } else {
             console.log(`【自定义脚本】[选项点击] 当前选项区有 ${optionsInThisArea.length} 个选项 (期望4个进行模式切换)。未执行模式自动切换逻辑。`);
        }
    ...
    window.DOMWatcherService.register({
        ...
        callback: function(controlsAreaElement, eventType) {
            console.log('【自定义脚本 via DOMWatcher】聊天控制区域 ('+ NATIVE_CONTROLS_AREA_SELECTOR +') 已 '+ eventType + ':', controlsAreaElement);
    ...
    window.DOMWatcherService.register({
        ...
        callback: function(optionsParentElement, eventType) {
            console.log('【自定义脚本 via DOMWatcher】选项父容器 ('+ OPTIONS_PARENT_SELECTOR +') 已 '+ eventType + ':', optionsParentElement);
            ...
            if (!listenedOptElements.has(optionsParentElement)) {
                ...
                console.log('【自定义脚本 via DOMWatcher】选项点击监听器已附加到新发现的 <opt>:', optionsParentElement);
            } else {
                // console.log('【自定义脚本 via DOMWatcher】选项点击监听器已存在于 <opt>:', optionsParentElement);
            }
        }
    });
    ...
    window.DOMWatcherService.register({
        ...
        callback: function(removedOptionsParentElement, eventType) {
            console.log('【自定义脚本 via DOMWatcher】选项父容器 ('+ OPTIONS_PARENT_SELECTOR +') 已 '+ eventType + ':', removedOptionsParentElement);
            if (listenedOptElements.has(removedOptionsParentElement)) {
                ...
                console.log('【自定义脚本 via DOMWatcher】选项点击监听器已从被移除的 <opt> 上卸载。');
            }
        }
    });

    console.log('【自定义脚本】已注入并配置 DOMWatcherService (v DOMWatcher 优化版)。现在可以通过 getCurrentSelectedModeText() 获取模式信息。');
